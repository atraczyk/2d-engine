...
void G_loadLevel(const char* file)
{
    ...
    for(int i = 0; i < world.level.enemySpawns.size(); i++)
    {
        world.enemies.push_back(Enemy((ENEMY_TYPE)world.level.enemySpawns[i].type,
                                ...
                                64.0f, 128.0f, world.level.enemySpawns[i].direction * ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    }
    ...
    sound.setReverb(false);
    sound.StopAll();
    sound.PlayMusic(SOUND_MUSIC1);
...
void U_Load_s1()
{
    world.level.ambientLighting = 1.0f;
    time.currentFrame = 0;
    camera.initialize(64, 0, game.sceneRect.w, game.sceneRect.h  );
    camera.isTargeting = true;
    world.level.layer[2].load("levels/map_s1_l1.lvl", &game.atlases.tiles, &camera);
    world.level.layer[3].initialize();
    world.level.layer[0].load("levels/map_s1_bg1.lvl", &game.atlases.bgs, &camera);
    world.level.layer[1].load("levels/map_s1_bg2.lvl", &game.atlases.bgs, &camera);
    world.constraints = WorldRect(	world.level.layer[2].w_tileSize.x,
                                    0,
                                    world.level.layer[2].t_mapSize.x * world.level.layer[2].w_tileSize.x - 2*world.level.layer[2].w_tileSize.x,
                                    world.level.layer[2].t_mapSize.y * world.level.layer[2].w_tileSize.y);
    world.level.dynTileMaps.Clear();
    player.initialize(240.0f, 120.0f, 64.0f, 128.0f, 8.0f, 8.0f, 16.0f, 8.0f, &game.atlases.sprites);
    player.hit = 0;
    player.hitEnemy = 0;
    world.enemies.clear();
    world.enemies.push_back(Enemy(ET_SHELL, 128.0f, 120.0f,64.0f, 128.0f, 0.0, 0.0f, 12.0f, 12.0f, 72.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_SHELL, 384.0f, 120.0f,64.0f, 128.0f, 0.0, 0.0f, 12.0f, 12.0f, 72.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_SHELL, 512.0f, 120.0f,64.0f, 128.0f, 0.0, 0.0f, 12.0f, 12.0f, 72.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 2596.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 2660.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
...
void U_Load_s2()
{
    world.level.ambientLighting = 0.25f;
    time.currentFrame = 0;
    camera.initialize(64, 0, game.sceneRect.w, game.sceneRect.h );
    camera.isTargeting = true;
    world.level.layer[2].load("levels/map_s2_l1.lvl", &game.atlases.tiles, &camera);
    world.level.layer[3].load("levels/map_s2_l2.lvl", &game.atlases.tiles, &camera);
    world.level.layer[0].load("levels/map_s2_bg1.lvl", &game.atlases.abgs, &camera);
    world.level.layer[1].initialize();
    world.constraints = WorldRect(world.level.layer[2].w_tileSize.x, 0,	world.level.layer[2].t_mapSize.x * world.level.layer[2].w_tileSize.x - 2*world.level.layer[2].w_tileSize.x,
        world.level.layer[2].t_mapSize.y * world.level.layer[2].w_tileSize.y);
    player.initialize(240.0f, 120.0f, 64.0f, 128.0f, 8.0f, 8.0f, 16.0f, 8.0f, &game.atlases.sprites);
    player.hit = 0;
    player.hitEnemy = 0;
    ...
    world.enemies.push_back(Enemy(ET_KOOPA, 440.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_GOOMBA, 540.0f, 120.0f, 64.0f, 128.0f, -ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 72.0f, 8.0f, &game.atlases.sprites));

    world.enemies.push_back(Enemy(ET_KOOPA, 1340.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 1404.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 1468.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 1532.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 1596.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 1660.0f, 120.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));

    world.enemies.push_back(Enemy(ET_KOOPA, 1468.0f, 632.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 1532.0f, 632.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 1596.0f, 632.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
    world.enemies.push_back(Enemy(ET_KOOPA, 1660.0f, 632.0f, 64.0f, 128.0f, ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));

    TileMap dynamic1,dynamic2,dynamic3;
    dynamic1.load("levels/2x1platform.lvl", &game.atlases.tiles, &camera);
    dynamic2.load("levels/2x1platform.lvl", &game.atlases.tiles, &camera);
    dynamic3.load("levels/3x1platform.lvl", &game.atlases.tiles, &camera);
    dynamic1.position = Vector2(1024,768);
    dynamic2.position = Vector2(2600,1700);
    dynamic3.position = Vector2(512,384);
    world.level.dynTileMaps.Clear();
    world.level.dynTileMaps.Add(dynamic1,[](unsigned int frame){float ftime = ((float)frame)*S_PER_GAME_UPDATE;
    return Vector2((int)(2*sgn(sinf((2*PI/2)*ftime))),(int)(-1*sgn(sinf((2*PI/8)*ftime))) );});
    world.level.dynTileMaps.Add(dynamic2,[](unsigned int frame){float ftime = ((float)frame)*S_PER_GAME_UPDATE;
    return Vector2(-2*sgn(sinf((2*PI/2)*ftime)),-1*sgn(sinf((2*PI/8)*ftime)) );});
    world.level.dynTileMaps.Add(dynamic3,[](unsigned int frame){float ftime = ((float)frame)*S_PER_GAME_UPDATE;
    return Vector2(2*sgn(sinf((2*PI/40)*ftime)),0);});

    sound.setReverb(true);
    sound.StopAll();
    sound.PlayMusic(SOUND_MUSIC2);
}

void U_Load_s3()
{
    world.level.ambientLighting = 0.85f;
    time.currentFrame = 0;
    camera.initialize(64, 0, game.sceneRect.w, game.sceneRect.h );
    camera.isTargeting = true;
    world.level.layer[2].load("levels/map_s3_back.lvl", &game.atlases.tiles, &camera);
    world.level.layer[3].load("levels/map_s3_front.lvl", &game.atlases.tiles, &camera);
    world.level.layer[0].load("levels/map_s2_bg1.lvl", &game.atlases.abgs, &camera);
    world.level.layer[1].initialize(0,0,0,0,0,0,0,NULL,0);
    world.constraints = WorldRect(world.level.layer[2].w_tileSize.x, 0,	world.level.layer[2].t_mapSize.x * world.level.layer[2].w_tileSize.x - 2*world.level.layer[2].w_tileSize.x,
        world.level.layer[2].t_mapSize.y * world.level.layer[2].w_tileSize.y);
    player.initialize(140.0f, 120.0f, 64.0f, 128.0f, 8.0f, 8.0f, 16.0f, 8.0f, &game.atlases.sprites);
    player.hit = 0;
    player.hitEnemy = 0;
    player.health = 3;
...
void U_Load_demo()
{
    ...
    game.clips.demo.AddEvent(0, []()
    {
        world.level.ambientLighting = 1.0f;
        camera.initialize(64, 0, game.sceneRect.w , game.sceneRect.h  );
        camera.isTargeting = false;
        world.level.layer[2].load("levels/map_s1_l1.lvl", &game.atlases.tiles, &camera);
        world.level.layer[3].load("levels/map_s1_l2.lvl", &game.atlases.tiles, &camera);
        world.level.layer[0].load("levels/map_s1_bg1.lvl", &game.atlases.bgs, &camera);
        world.level.layer[1].load("levels/map_s1_bg2.lvl", &game.atlases.bgs, &camera);
        world.constraints = WorldRect(world.level.layer[2].w_tileSize.x, 0,	world.level.layer[2].t_mapSize.x * world.level.layer[2].w_tileSize.x - 2*world.level.layer[2].w_tileSize.x,
            world.level.layer[2].t_mapSize.y * world.level.layer[2].w_tileSize.y);
        ...
        player.hitEnemy = 0;
        world.enemies.clear();
        world.enemies.push_back(Enemy(ET_SHELL, 340.0f, 120.0f,64.0f, 128.0f, 0.0, 0.0f, 12.0f, 12.0f, 72.0f, 8.0f, &game.atlases.sprites));
        world.enemies.push_back(Enemy(ET_GOOMBA, 740.0f, 120.0f, 64.0f, 128.0f, -ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 72.0f, 8.0f, &game.atlases.sprites));
        world.enemies.push_back(Enemy(ET_KOOPA, 2540.0f, 120.0f, 64.0f, 128.0f, -ENEMY1_X_SPEED, 0.0f, 8.0f, 8.0f, 32.0f, 8.0f, &game.atlases.sprites));
...
void G_initializeGame()
{
    ...
    game.atlases.sprites.initialize(&game.textures.sprites, 32, 16, 16, 32);
    game.atlases.tiles.initialize(&game.textures.tiles, 16, 16, 16, 16);
    game.atlases.bgs.initialize(&game.textures.bgs, 2, 2 , 512, 432);
    game.atlases.abgs.initialize(&game.textures.abgs, 2, 2 , 512, 432);
    game.atlases.font.initialize(&game.textures.font, 16, 16, 8, 8);
...
{
    game.textures.sprites = loadBMPTexture_ARGB("textures/sprite01.bmp");
    game.textures.tiles = loadBMPTexture_ARGB("textures/tiles03.bmp");
    game.textures.bgs = loadBMPTexture_ARGB("textures/bg01.bmp");
    game.textures.abgs = loadBMPTexture_ARGB("textures/abg01.bmp");
    game.textures.font = loadBMPTexture_ARGB("textures/font_8x8.bmp");
    game.textures.light = loadBMPTexture_ARGB("textures/light.bmp");
    game.textures.cursor = loadBMPTexture_ARGB("textures/cursor.bmp");
    game.textures.slider = loadBMPTexture_ARGB("textures/slider.bmp");
}
...
void I_updateGameInput(Input* pinput)
{
    ...
    if(	pinput->keyboard.keyReleased(VK_NUMPAD3))
        U_Load_s3();
    if(	pinput->keyboard.keyReleased(VK_NUMPAD4))
        G_loadLevel("levels/stage3.lvl");
    if(	pinput->keyboard.keyReleased(VK_NUMPAD5))
...
void P_cleanup()
{
    ...
    glDeleteTextures(1, &game.textures.abgs);
