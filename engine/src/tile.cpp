...
void TileMap::save(const char *filename) {
  ...
  stringstream ss;
  if (file.is_open()) {
    file << has_c_Map << " " << position.x << " " << position.y << " "
         << w_tileSize.x << " " << w_tileSize.y << " " << w_depth << endl;
    for (std::uint16_t x = 0; x < imageLayer.size(); x++) {
      for (std::uint16_t y = 0; y < imageLayer[0].size(); y++) {
    ...
    if (has_c_Map) {
      for (std::uint16_t x = 0; x < collisionLayer.size(); x++) {
        for (std::uint16_t y = 0; y < collisionLayer[0].size(); y++) {
...
void TileMap::load(const char *filename, TileAtlas *ptexatlas,
  ...
  if (file.is_open()) {
    ...
    ssf >> has_c_Map >> position.x >> position.y >> w_tileSize.x >>
        w_tileSize.y >> w_depth;
      ...
      while (ss >> index) {
        ...
        if (animated)
          animatedTiles.push_back(
              AnimatedTile(Point(row, col), tileAnimations[index - 240]));
...
void TileMap::initialize(float x, float y, int w, int h, float tilewidth,
                         float tileheight, float depth, TileAtlas *ptexatlas,
                         bool has_c_map, int itype, int ctype) {
  ...
  for (int col = 0; col < w; col++) {
    ...
    for (int row = 0; row < h; row++) {
      ...
      if (has_c_Map)
        collisionLayer.back().push_back(
            CTile(ctype, (ctype != 0), entry_type[ctype], NULL));
    }
  }
}
...
void TileMap::update() {
  ...
  for (unsigned int i = 0; i < animatedTiles.size(); i++) {
    ...
    imageLayer[p.x][p.y].bounce = imageLayer[p.x][p.y].bounce < 0.5f
                                      ? 0
                                      : imageLayer[p.x][p.y].bounce - 1.0f;
    animatedTiles[i].animation.update();
    imageLayer[p.x][p.y].tileIndex =
        animatedTiles[i].animation.GetCurrentIndex();
...
void TileMap::draw(Camera *camera) {
  ...
  max.x = std::min(TopRight.x + 1, t_mapSize.x);
  min.y = std::max(0, BottomLeft.y - 1);
  max.y = std::min(TopRight.y + 1, t_mapSize.y);
  ...
  for (int y = min.y; y < max.y; y++) {
    for (int x = min.x; x < max.x; x++) {
      if (imageLayer[x][y].tileIndex < 0)
        continue;
      float b = -0.64f * ((imageLayer[x][y].bounce - 5) *
                          (imageLayer[x][y].bounce - 5)) +
                16.0f;
