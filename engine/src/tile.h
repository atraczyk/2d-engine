...
public:
  int tileIndex;
  bool isCollidable;
  std::uint16_t collisionFlags;
  void (*function) ();

  CTile (int index = 0, bool c = false, std::uint16_t l = 0x0000,
         void (*f) () = NULL)
  {
    tileIndex = index;
    isCollidable = c;
...
public:
  ...
  unsigned char currentAtlasFrame;
  ...
  void
  addFrame (int index, unsigned int delay)
  {
...
public:
  ...
  void
  load (const char *filename, TileAtlas *ptexatlas, Camera *camera);
  void
  initialize (float x = 0, float y = 0, int w = 0, int h = 0,
              float tilewidth = 0, float tileheight = 0, float depth = 0,
              TileAtlas *ptexatlas = NULL, bool has_c_map = false,
              int itype = -1, int ctype = 0);
  void
  adjustPosition (Camera *pcamera);
  void
  update ();
  void
  draw (Camera *camera);
  void
  DrawTile (Camera *camera, ScreenRect dstRect, int index);
};
...
public:
  ...
  void
  Add (TileMap tilemap, Vector2 (*_function) (unsigned int))
  {
    count++;
  ...
  void
  update (unsigned int currentFrame)
  {
