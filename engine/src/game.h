...
#include "entity.h"
#include "event.h"
#include "game_time.h" // Contains GameTime class definition
#include "input.h"
#include "menu.h"
#include "objects.h"
#include "program.h"
...
R_drawMenus ();
void
R_drawSky (RenderTarget target);
void
R_drawSceneLights (TileMap *ptilemap, float ls);
void
R_drawSelectedMenuItem (MenuItem item, float tx, float ty, float tw, float th);

void
G_initializeGame ();
void
G_update (double elapsedTime);
void
G_initializeEvents ();
bool
G_playEvent (std::string name, bool loop);
bool
G_stopEvent (std::string name);
void
G_initializeMenus ();

void
I_updateGameInput (Input *pinput);
void
I_updateProgramInput (Input *pinput);
void
I_updateMenuInput (Input *pinput);
void
I_resetControls ();
void
I_zeroGameInput ();
...
void
GL_loadShader (unsigned int &window, const char *vsfile, const char *fsfile);
